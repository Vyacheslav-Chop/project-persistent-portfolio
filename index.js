...
`,c.appendChild(l)});const s=document.querySelector("[data-load-more]");t>=e.length?s.style.display="none":s.style.display="block"}document.querySelector("[data-load-more]").addEventListener("click",()=>{t+=3,o()}),o()});async function W(){try{return(await f.get("https://portfolio-js.b.goit.study/api/reviews")).data}catch{return v.error({title:"Error",message:"Failed to load reviews from server."}),null}}function G(e){const t=document.getElementById("reviews-list");if(t.innerHTML="",!e||e.length===0){t.innerHTML='<li class="swiper-slide">Not found</li>';return}e.map(({author:o,avatar_url:c,review:r})=>{const s=document.createElement("li");s.className="swiper-slide",s.innerHTML=`
    ...
    `,t.appendChild(s)})}function z(){new $(".reviews-swiper",{slidesPerView:1,spaceBetween:20,keyboard:{enabled:!0},breakpoints:{1280:{slidesPerView:2}},navigation:{nextEl:".swiper-button-next",prevEl:".swiper-button-prev"}})}async function K(){const e=await W();G(e),z()}K();const M=document.querySelector(".work-together-form"),p=document.querySelector("#user-email"),_=document.querySelector("#user-message"),b=document.querySelector(".email-error");M.addEventListener("submit",Z);function Z(e){e.preventDefault();const t=/^\w+(\.\w+)?@[a-zA-Z_]+?\.[a-zA-Z]{2,3}$/,o=p.value.trim(),c=_.value.trim();if(t.test(o))b.classList.add("hidden");else{b.classList.remove("hidden"),b.style.marginBottom="16px",p.classList.add("error"),p.style.color="red",p.style.marginBottom="5px";return}U({email:o,comment:c})}function U(e){f.post("https://portfolio-js.b.goit.study/api/requests",e).then(t=>{Y(t.data.title,t.data.message),M.reset()}).catch(t=>{v.error({title:"Error",timeout:1e4,message:"Something went wrong. Please check your data and try again.",position:"topRight"})})}function Y(e,t){const o=document.createElement("div");o.className="work-together-backdrop",o.innerHTML=`
